...
session = Session()
qry = session.query(models.Advertisement.id, models.Advertisement.description, models.Advertisement.characteristics).all()

eprint("Transform descriptions...")
all_words = []
for q in qry:
    words = set(re.split(' ', re.sub(remove_tokens, ' ', (str(q.description) + str(q.characteristics)).lower())))
    words = set([w for w in words if w not in remove_words and len(w) > 1])

    all_words += set([replacements.get(word, word) for word in words])

eprint("Count words...")
counted = Counter(all_words).most_common(10000)
